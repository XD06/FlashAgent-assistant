...
| 引擎 | 实现 | 精度 | 成本 |
|---|---|---|---|
| `system`（默认，仅 Windows） | `Windows.Media.Ocr`，C# helper 由本机 .NET Framework csc.exe 现场编译（`src/main/ocr/winrtHelper.ts`），与 CaptureScreen.exe 同一套零依赖模式 | 英文/数字基本全对；中文 gist 级可用，小字有部首级误识（实测见 `ocr-winrt-test-result.md`） | +0 MB、~100-300ms、无常驻内存（每次识别一个短命进程，PNG 走 stdin 不落盘） |
